...
- Domain collection の不変条件は、条件集合の重複除去、比較規約、空集合可否などを扱う責務とする。文字列集合の比較は、個別仕様で明記しない限り `StringComparer.Ordinal` 相当の case-sensitive 比較とする
- merge signature / comparison の安定化は、merge の grouping、containment、差分判定、ログ用 signature のために集合を安定順序へ並べる処理とする。これは保存値や表示値を正規化する処理ではない
- export formatting は、人が読む CSV 表記へ戻す処理とする。`any` 相当範囲の `any` 表記、protocol 番号の protocol 名表記、CIDR 化できるアドレス範囲の CIDR 表記、出力順の安定化は exporter 側の責務とする
