...
- 名前付きサービス定義の `TCP` `UDP` `ICMP` `SCTP` は IP プロトコル番号へ正規化する
- 名前付きサービス定義の `any` はポート範囲 `1-65535` へ正規化する
...
- アドレス参照は`any`、名前付きアドレス定義、アドレス グループ、直接値の順に解決対象とする
- サービス参照は`any`、名前付きサービス定義、サービス グループ、直指定、Kind 指定 (`application-default` など) の順に解決対象とする
- サービス参照そのものの `any` は、ServiceValue 上ではプロトコル `0-255`、送信元ポート `0-65535`、宛先ポート `0-65535`、`Kind = null` として扱う
- 名前付きサービス定義または直指定サービスの各軸に現れる `any` は、プロトコルでは `0-254`、ポートでは `1-65535` として扱う
