...
- Domain 内で protocol alias を解釈する場合、`tcp` `udp` `icmp` `sctp` の小文字だけを IP プロトコル番号へ正規化する
- Domain 内で名前付きサービス定義または直指定サービスの protocol 軸に現れる `any` は小文字のみを特別扱いし、`ANY` は許可しない
...
本仕様では、正規化を次の責務に分けて扱います。

- CSV 取り込み正規化は、製品固有の入力表記を Domain が扱いやすい入力値へ変換する処理とする。Palo Alto CSV の名前付きアドレス定義の単一 IPv4 を `/32` へ変換する処理、名前付きサービス定義の protocol alias を IP プロトコル番号へ変換する処理、名前付きサービス定義の port `any` / `ANY` や `0-xxxx` 範囲を内部処理用ポート範囲へ変換する処理は、CSV repository 側の責務とする
- Domain 値解釈は、取り込み済みまたは解決済みの値を Domain の値として検証し、内部表現へ変換する処理とする。アドレス参照の小文字 `any`、サービス参照の小文字 `any`、アドレス範囲、サービス範囲、protocol / port 範囲の補正は Domain 側の責務とする
