...
- [6. 共通仕様](#6-共通仕様)
  ...
  - [6.6. 正規化・解釈・検証責務](#66-正規化解釈検証責務)
  - [6.7. Canonical order](#67-canonical-order)
- [7. 機能仕様](#7-機能仕様)
...
値だけを表す入力軸では、取り込み時に内部処理用の alias 正規化を行う場合があります。
...
- 直指定サービス `<protocol> <source-port> <destination-port>` の各軸に現れる `any` は、小文字のみを取り込み時の正規化対象とする
- 直指定サービスは、3 要素すべてが canonical な protocol / port 数値範囲として妥当な場合だけ直指定として扱う
- `tcp ANY 80` のように 3 要素に分割できても canonical direct service として妥当でない未解決サービス参照は、warning を出したうえで Kind 指定として扱う
- 名前付きサービス定義の source port / destination port は、Palo Alto CSV 取り込み時に内部処理用として `any` / `ANY` を `1-65535` へ正規化する
- protocol alias (`tcp` `udp` `icmp` `sctp`) や protocol 軸の `any` は、取り込み時に Domain が扱う数値範囲へ正規化する
- Domain 内で名前付きサービス定義または direct service としてパースする値の protocol 軸に alias や未正規化の `any` が残っている場合は不正値として扱う
...
本仕様では、正規化、解釈、検証を次の責務に分けて扱います。

- Infra 取り込み正規化は、製品固有の入力表記を Domain が扱いやすい入力値へ変換する処理とする。Palo Alto CSV の名前付きアドレス定義およびセキュリティ ポリシーの直接アドレス参照の単一 IPv4 を `/32` へ変換する処理、名前付きアドレス定義の host bit 付き CIDR をネットワーク境界へ丸める処理、名前付きサービス定義および直指定サービスの protocol alias を IP プロトコル番号へ変換する処理、仕様で許容する protocol / port 軸の `any` 表記を内部処理用範囲へ変換する処理、port `0` 系入力を補正または除去する処理は、Infra repository 側の責務とする
- Domain 値解釈は、取り込み済みまたは解決済みの値を Domain の値として検証し、正しい表現を内部表現へ変換する処理とする。Domain は入力表記を「直してから通す」責務を持たない。正規化済みであるべき値に `/32` なしの単一 IPv4、host bit 付き CIDR、protocol alias、protocol `255` 終端、port `any`、port `0` などが残っている場合、Domain は警告で継続せずエラーとして扱う
- atomize workflow は Domain の address 解釈エラーを補正しないが、`CN` / `JP` など IPv4 条件へ展開できない address がグループ解決後に現れる運用を考慮し、該当ポリシー全体を warning 付きでスキップして続行する
- 未解決 service 参照の分類では、canonical direct service として検証できた値だけを direct service として扱う。3 要素に分割できても canonical direct service として妥当でない値は、Domain 値の補正対象ではなく Kind 指定候補として扱い、warning を出したうえで Kind 指定へフォールバックする
- Domain が扱ってよい `any` は、参照値として仕様定義した小文字 `any` と内部表現に限定する。Domain の `any` や Kind 用 sentinel の扱いは内部意味の解釈であり、外部入力の表記ゆれや軸値 `any` の補正として使ってはならない。外部入力の alias / 表記ゆれ / sentinel 相当表現は Infra で正規化または拒否する
- Domain collection の不変条件は、条件集合の重複除去、比較規約、空集合可否、import 後の canonical order などを扱う責務とする。文字列集合の比較は、個別仕様で明記しない限り `StringComparer.Ordinal` 相当の case-sensitive 比較とする
- merge signature / comparison の安定化は、merge の grouping、containment、差分判定、ログ用 signature のために集合を canonical order へ並べる処理とする
- export formatting は、人が読む CSV 表記へ戻す処理とする。`any` 相当範囲の `any` 表記、protocol 番号の protocol 名表記、CIDR 化できるアドレス範囲の CIDR 表記は exporter 側の責務とする。条件集合の出力順は Domain の canonical order に従う

application 値は製品・環境ごとの識別子として入力表記を保持するため、CSV 取り込み正規化および Domain 値解釈で表記を変更しません。
`any` の包含判定は containment / shadow 判定の条件評価に限定し、保存値、集合値、出力値の正規化としては扱いません。

### 6.7. Canonical order

`import` は元データ取り込みとして、CSV 内の参照リスト順を保持します。canonical order は、import 後に解決または生成された Domain 条件値、SQLite JSON、export、merge / test の診断表示へ適用します。

- 文字列集合は `StringComparer.Ordinal` 相当の昇順で並べる
- アドレス条件値は `Start`、`Finish` の昇順で並べる
- サービス条件値は、サービス参照そのものの組み込み `any`、`Kind` 指定、3 軸指定サービスの順で並べる
- `Kind` 指定同士は `Kind` の Ordinal 昇順で並べる
- 3 軸指定サービス同士は、protocol number start/end、destination port start/end、source port start/end の順で並べる
- ポリシー行の tie-break や診断表示で条件軸の順序が必要な場合は、順序に意味のある index / zone / action / traceability を優先し、その後に宛先アドレス、サービス、アプリケーション、送信元アドレスの順を使う

大量データのルール行ソートは Domain では行わず、SQLite repository の `ORDER BY` に任せます。Domain runner は、repository から順序契約を満たす列が渡される前提で処理します。
...
- 名前付きアドレス定義およびセキュリティ ポリシーの直接アドレス参照の単一 IPv4 は `/32` へ正規化する
- 名前付きアドレス定義の host bit 付き CIDR はネットワーク境界へ丸める
- セキュリティ ポリシーのアドレス参照欄の小文字 `any` は、後続解決で `0.0.0.0/0` として扱う
- 名前付きサービス定義の protocol alias `TCP` `UDP` `ICMP` `SCTP` は、CSV 取り込み時に大文字小文字を区別せず IP プロトコル番号へ正規化する
- 名前付きサービス定義の source port / destination port の `any` は、大文字小文字を区別せずポート範囲 `1-65535` へ正規化する
- セキュリティ ポリシーのサービス参照欄は service object / service group 名の可能性を保持するため、3 要素に分割できるだけでは import エラーにしない
- セキュリティ ポリシーの 3 要素直指定サービスとして正規化できる値では、protocol alias `tcp` `udp` `icmp` `sctp` を IP プロトコル番号へ正規化し、protocol / source port / destination port 軸に現れる小文字 `any` を protocol では `0-254`、port では `1-65535` へ正規化する
- 名前付きサービス定義の source port / destination port の `0` / `0-0` は範囲要素単位で除去する。除去によりポート軸が空になる場合は CSV 読み取りエラーとする
- 名前付きサービス定義の source port / destination port の `0-xxxx` (`xxxx >= 1`) は `1-xxxx` へ補正する
...
- 名前付きサービス定義または直指定サービスの各軸に現れる小文字 `any` は、取り込み時にプロトコルでは `0-254`、ポートでは `1-65535` へ正規化済みであることを前提とする
- サービス参照欄の `ANY` は組み込み `any` ではなく、完全一致するサービス定義 / サービス グループがなければ Kind 指定として扱う
- 直指定サービスは、protocol が `0-254`、source port / destination port が `1-65535` の canonical な数値範囲としてパースできる場合だけ扱う
- 3 要素に分割できる未解決サービス参照であっても、canonical direct service としてパースできない場合は warning を出し、Kind 指定へフォールバックする
- 3 軸指定サービスの protocol 範囲は `0-254` を有効範囲とする。組み込みサービス `any` と Kind 指定の内部表現を除き、protocol `255` が Domain に到達した場合は解決エラーとして扱う
- 名前付きサービス定義または直指定サービスの port は `1-65535` を有効範囲とする。組み込みサービス `any` と Kind 指定の内部表現を除き、port `0` が Domain に到達した場合は解決エラーとして扱う
- グループの再帰参照を検知した場合は解決エラーとして atomize を中断する
...
- `192.168.1.10` のように `/32` を省略した単一 IPv4 が Domain 値解釈に到達した場合、Domain parser はエラーとして扱い、atomize workflow は該当ポリシーを warning 付きでスキップする
- `192.168.1.10/24` のように host bit を含む CIDR が Domain 値解釈に到達した場合、Domain parser はエラーとして扱い、atomize workflow は該当ポリシーを warning 付きでスキップする
- `CN` や `JP` など、IPv4 条件へ展開できない address object / group 解決結果が atomize に到達した場合も、該当ポリシーを warning 付きでスキップする
- 要素数が `threshold` 以上の範囲は範囲のまま保持する
- atomic 化はアクション種別に関係なく実施する

#### 7.2.5. エラー時の扱い

- アドレス条件を IPv4 範囲として展開できない場合、atomize は値だけを落とさず、該当ポリシー全体を warning 付きでスキップして続行する
- サービス定義値の strict parse エラーやグループ再帰など、address 展開不能以外の Domain 例外は atomize を中断してエラー終了する
...
atomic の通常読み出しおよび export に使う標準順は、`OriginalIndex`、`FromZone`、`ToZone`、`Action`、宛先アドレス、サービス、アプリケーション、送信元アドレス、保存順とします。条件値の比較キーは [6.7. Canonical order](#67-canonical-order) に従い、SQLite 実装では SQL の `ORDER BY` で並べます。
...
merge に渡す atomic は、`FromZone`、`ToZone`、`Service.Kind`、`OriginalIndex`、宛先アドレス、サービス、アプリケーション、送信元アドレス、`Action`、保存順の順で整列されているものとします。
`Service.Kind` は merge partition key であるため、サービス条件値全体の canonical order より前に置き、同じ merge パーティションが連続することを優先します。
...
shadow 判定は、同一 merge パーティション内で、先行する 1 件の non-shadowed atomic 候補が後続 atomic 候補を完全包含するかどうかで判定します。

ここでいう完全包含は、先行 atomic 候補のアプリケーション、送信元アドレス、宛先アドレス、サービスの各条件が、後続 atomic 候補の対応する条件をすべて覆う状態を指します。

- shadow 判定は、1 件の先行 atomic 候補による単体包含だけを対象とする
- 複数の先行 atomic 候補を合わせると後続 atomic 候補全体を覆える場合でも、単体で完全包含する先行 atomic 候補が存在しない限り shadowed とはみなさない
- アプリケーションの shadow 判定では、先行 atomic 候補のアプリケーションが `any` の場合、後続 atomic 候補の個別アプリケーションを shadow するとみなす
- shadowed な後続 atomic 候補は出力対象から外す
- ただし、元インデックス範囲と元ポリシー名は、shadow を作った先行 atomic 候補側へ吸収する
- shadow 判定はアクション差異があっても適用される

先行 atomic 候補集合による到達不能性、たとえば複数の CIDR / IP range / port range の和集合が後続 atomic 候補の範囲全体を覆うケースは、本製品の shadow 判定対象外とします。

例: `threshold = 3` で atomize した場合

| 元インデックス | 元ルール | 宛先アドレス | atomize 後の宛先アドレス |
| --- | --- | --- | --- |
| 10 | rule-a | `192.168.0.1,192.168.0.2,192.168.0.3` | `192.168.0.1` / `192.168.0.2` / `192.168.0.3` |
| 20 | rule-b | `192.168.0.1-192.168.0.3` | `192.168.0.1-192.168.0.3` |

`rule-a` と `rule-b` の FromZone、ToZone、Application、Service が同じで、`rule-a` が `rule-b` より先行している場合、ファイアウォールの first-hit 評価としては `rule-b` の宛先範囲全体が `rule-a` によって到達不能になります。

ただし、この例では `rule-a` 由来の各 atomic 候補は、いずれも `rule-b` 由来の `192.168.0.1-192.168.0.3` 全体を単体では包含しません。

- `192.168.0.1` は `192.168.0.1-192.168.0.3` 全体を包含しない
- `192.168.0.2` は `192.168.0.1-192.168.0.3` 全体を包含しない
- `192.168.0.3` は `192.168.0.1-192.168.0.3` 全体を包含しない

そのため、3 件の先行 atomic 候補を合わせれば後続範囲を被覆できる場合でも、`rule-b` 由来の atomic 候補は shadowed とはみなしません。

なお、この例が Allow ルールであり、`Action` と `GroupId` も一致する場合、後続の Allow ルールの完全包含統合では `rule-b` 由来の範囲候補が `rule-a` 由来の単一 IP 候補をそれぞれ包含します。
そのため、最終的な merged 出力では、3 件の単一 IP 候補と範囲候補は別々には残らず、`192.168.0.1-192.168.0.3` を持つ 1 件へ集約されます。
これは first-hit 評価上の shadow 判定ではなく、同一アクション内の重複除去として扱います。
...
完全に包含するルールの統合は、Allow 候補同士の重複除去として扱います。
この重複除去では、1 件の候補が別の 1 件の候補を完全包含する単体包含だけを対象とします。
複数候補の和集合による被包含判定は行いません。
...
- atomic / merged CSV の条件値集合は、[6.7. Canonical order](#67-canonical-order) に従って出力する
...
- `source_address_finish`
- `to_zone`
- `destination_address_start`
- `destination_address_finish`
- `application`
- `service_protocol_start`
- `service_protocol_finish`
- `service_source_port_start`
- `service_source_port_finish`
- `service_destination_port_start`
- `service_destination_port_finish`
- `service_kind`
- `action`
- `group_id`
- `original_index`
...
- atomic は merge 用順序で走査する。SQLite 実装では `FromZone`、`ToZone`、`Service.Kind`、`OriginalIndex`、宛先アドレス、サービス、アプリケーション、送信元アドレス、`Action`、保存順で読み取る
- atomic の走査順は shadowed 分類のために merge partition 連続性を優先する
...
- test runner は atomic / merged の入力列を再ソートしない

#### 7.5.5. 含有判定

- 各 atomic について、比較対象 merged の先頭から順に「含まれる」 merged を探索する
...
- Palo Alto CSV 入力では単一 IPv4、CIDR、IPv4 範囲を扱う
- Domain の canonical address 値は `any`、CIDR、IPv4 範囲とし、単一 IPv4 は `/32` CIDR として表現する
...
- 3 要素に分割できない未定義サービス参照は Kind 指定として扱う
- 3 要素に分割できる未定義サービス参照でも、canonical direct service としてパースできない場合は warning を出して Kind 指定として扱う
...
- アドレス値が IPv4 条件へ展開不能な場合、`atomize` は該当ポリシーを warning 付きでスキップする
- サービス定義値など、address 展開不能以外の Domain 値が解釈不能な場合、`atomize` はエラー終了する
