...
- アドレス オブジェクトの単一 IPv4 は `/32` へ正規化する
- アドレス `any` は後続解決で `0.0.0.0/0` として扱う
- サービス オブジェクトの `TCP` `UDP` `ICMP` `SCTP` は IP プロトコル番号へ正規化する
- サービス オブジェクトの `any` はポート範囲 `1-65535` へ正規化する
- `0-xxxx` 形式のポート範囲は `1-xxxx` へ補正する
- セキュリティ ポリシーの `ルールの使用状況 内容` に `AAA|BBB` 形式がある場合、`AAA` を `group_id` として採用する

#### 7.1.5. 永続化動作

- オブジェクト系テーブルと imported policy テーブルを同一トランザクション内で全件置換する
...
- アドレス参照は`any`、アドレス オブジェクト、アドレス グループ、直接値の順に解決対象とする
- サービス参照は`any`、サービス オブジェクト、サービス グループ、直指定、Kind 指定 (`application-default` など) の順に解決対象とする
- サービス参照そのものの `any` は、ServiceValue 上ではプロトコル `0-255`、送信元ポート `0-65535`、宛先ポート `0-65535`、`Kind = null` として扱う
- サービス オブジェクトまたは直指定サービスの各軸に現れる `any` は、プロトコルでは `0-254`、ポートでは `1-65535` として扱う
- プロトコル範囲の終端 `255` は `254` へ補正し、ポート範囲の開始 `0` は `1` へ補正する。カンマ区切り複数値では各要素ごとに補正する
- グループの再帰参照を検知した場合は当該ポリシーをスキップする

#### 7.2.4. 範囲展開ルール

- アドレス範囲、プロトコル範囲、ポート範囲は、要素数が `threshold` 未満のときだけ単一値へ分解する
