...
internal static class EntityValueCodec
{
    ...
    /// <summary>
    /// アドレス定義の列挙を JSON 文字列へ変換します。
    /// </summary>
    /// <param name="values">変換するアドレス定義の列挙。</param>
    /// <returns>JSON 文字列。</returns>
    public static string SerializeSecurityPolicyAddresses(IEnumerable<AddressObject> values)
    {
        var payload = values
            .Select(value => string.IsNullOrWhiteSpace(value.Value) ? value.Name : value.Value)
            .ToArray();
    ...
    /// <summary>
    /// JSON 文字列からアドレス定義一覧を復元します。
    /// </summary>
    /// <param name="value">復元元の JSON 文字列。</param>
    /// <returns>復元したアドレス定義一覧。</returns>
    public static IReadOnlyList<AddressObject> DeserializeSecurityPolicyAddresses(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return [];
    ...
    /// <summary>
    /// サービス定義の列挙を JSON 文字列へ変換します。
    /// </summary>
    /// <param name="values">変換するサービス定義の列挙。</param>
    /// <returns>JSON 文字列。</returns>
    public static string SerializeSecurityPolicyServices(IEnumerable<ServiceObject> values)
    {
        var payload = values
            .Select(value => value.Kind is null
                ? new Dictionary<string, object?>
                {
                    ["proto"] = ParseJsonNumericOrTextValue(value.Protocol),
                    ["src"] = ParseJsonPortValue(value.SourcePort),
                    ["dst"] = ParseJsonPortValue(value.DestinationPort)
                }
                : new Dictionary<string, object?>
                {
                    ["proto"] = ParseJsonNumericOrTextValue(value.Protocol),
    ...
    /// <summary>
    /// JSON 文字列からサービス定義一覧を復元します。
    /// </summary>
    /// <param name="value">復元元の JSON 文字列。</param>
    /// <returns>復元したサービス定義一覧。</returns>
    public static IReadOnlyList<ServiceObject> DeserializeSecurityPolicyServices(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            return [];
        ...
        {
            throw new FormatException("services_json must be a JSON array.");
        }

        var items = new List<ServiceObject>();
        foreach (var element in document.RootElement.EnumerateArray())
        {
            if (element.ValueKind == JsonValueKind.String)
            {
                var serviceName = element.GetString() ?? string.Empty;
                items.Add(new ServiceObject
                {
                    Name = serviceName,
            ...
            {
                throw new FormatException("services_json contains an unsupported element.");
            }

            items.Add(new ServiceObject
            {
                Name = string.Empty,
                Protocol = ReadRequiredStringLikeValue(element, "proto"),
                SourcePort = ReadRequiredStringLikeValue(element, "src"),
                DestinationPort = ReadRequiredStringLikeValue(element, "dst"),
                Kind = ReadOptionalStringValue(element, "kind")
            });
        }
    ...
    private static object ParseJsonPortValue(string value)
    {
        return int.TryParse(value, out var port) ? port : value;
    }

    private static object ParseJsonNumericOrTextValue(string value)
    {
        return int.TryParse(value, out var numericValue) ? numericValue : value;
    }

    private static string ReadRequiredStringLikeValue(JsonElement element, string propertyName)
    {
        if (!element.TryGetProperty(propertyName, out var property))
        {
            throw new FormatException($"services_json object must contain '{propertyName}'.");
        }
