...
- shadowed 判定は、同一 merge パーティション内で、先行する 1 件の atomic 候補が後続 atomic 候補を完全に覆う場合だけ成立します。
- 複数の先行 atomic 候補を合わせれば後続 atomic 候補を覆える場合でも、単体で完全に覆う候補がない場合は shadowed とはみなしません。
...
例として、`threshold = 3` のとき、先行ルールの宛先 `192.168.0.1,192.168.0.2,192.168.0.3` は 3 件の atomic に分かれます。
一方、後続ルールの宛先 `192.168.0.1-192.168.0.3` は要素数が threshold 以上なので範囲のまま残ります。

この場合、先行 atomic 3 件の集合は後続範囲を覆いますが、先行 atomic 1 件だけで後続範囲全体を覆うものはありません。
そのため、現行仕様では後続ルールを shadowed とはみなしません。
ただし、同じ Allow ルールで `Action` と `GroupId` も一致する場合、後続の重複除去では範囲側が単一 IP 側を包含するため、最終的な merged 出力では `192.168.0.1-192.168.0.3` を持つ 1 件へ集約されます。
